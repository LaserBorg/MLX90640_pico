...
#include <mutex>
std::mutex mtx; // Mutex for synchronizing access to the values array
...
extern "C" void core1_entry() {
    while (true) {
        ...
        mtx.lock(); // Lock the mutex before accessing the values array
        ...
        mtx.unlock(); // Unlock the mutex after we're done with the values array
    }
}
